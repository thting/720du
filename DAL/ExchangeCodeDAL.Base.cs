...
{
    /// <summary>
    /// 数据表ExchangeCode的数据操作类
    /// </summary>
    public partial class ExchangeCodeDAL
    {
        #region ConstVariables
        private const string C_TABLE_NAME = "ExchangeCode";
        private const string C_SP_EXCHANGECODE_FIELDS = "[Id],[ShopId],[ExchangeCode]";
        private const string C_SP_EXCHANGECODE_INSERT = "INSERT INTO [ExchangeCode]([ShopId],[ExchangeCode]) VALUES(@ShopId,@ExchangeCode);SET @Id = SCOPE_IDENTITY();";
        private const string C_SP_EXCHANGECODE_UPDATE = "UPDATE [ExchangeCode] SET [ShopId]=@ShopId,[ExchangeCode]=@ExchangeCode WHERE [Id] = @Id";
        private const string C_SP_EXCHANGECODE_DELETE = "DELETE [ExchangeCode] WHERE [Id] = @Id";
        private const string C_SP_EXCHANGECODE_GET = "SELECT [Id],[ShopId],[ExchangeCode] FROM [ExchangeCode] WHERE [Id] = @Id";
        #endregion
        ...
        public static void Delete(int id)
        {
            List<SqlParameter> commandParms = new List<SqlParameter>();
            commandParms.Add(SqlHelper.CreateParam("@Id", SqlDbType.Int, 0, ParameterDirection.Input, id));
            SqlHelper.ExecuteNonQuery(ConnectionString, CommandType.Text, C_SP_EXCHANGECODE_DELETE, commandParms);
        }
        ...
        public static DataTable SearchDT(string whereClause, string dataFields, string orderBy, int startRowIndex, int maximumRows)
        {
            if (dataFields.Trim() == "*")
            {
                dataFields = C_SP_EXCHANGECODE_FIELDS;
            }
        ...
        /// <summary>
        /// 按条件查询数据表,返回 Entity.ExchangeCodeEntity 数据集
        /// </summary>
        /// <param name="whereClause">SQL条件语句(可为空)，不须带"Where"关键字</param>
        /// <param name="orderBy">SQL排序语句(不能为空)，不须带"Order By"关键字</param>
        /// <param name="startRowIndex">记录开始索引，从0开始</param>
        /// <param name="maximumRows">返回记录数量</param>
        public static List<Entity.ExchangeCodeEntity> Search(string whereClause, string orderBy, int startRowIndex, int maximumRows)
        {
            List<Entity.ExchangeCodeEntity> list = new List<Entity.ExchangeCodeEntity>();
            using (SqlDataReader reader = SqlListHepler.SearchDataReader(ConnectionString, C_TABLE_NAME, C_SP_EXCHANGECODE_FIELDS, whereClause, orderBy, startRowIndex, maximumRows))
            {
                while (reader.Read())
                {
        ...
        /// <param name="orderBy">SQL排序语句(可为空)，不须带"Order By"关键字</param>
        /// <param name="rowsToReturn">返回记录数量</param>
        public static List<Entity.ExchangeCodeEntity> Search(string whereClause, string orderBy, int rowsToReturn)
        {
            List<Entity.ExchangeCodeEntity> list = new List<Entity.ExchangeCodeEntity>();
            using (SqlDataReader reader = SqlListHepler.SearchDataReader(ConnectionString, C_TABLE_NAME, C_SP_EXCHANGECODE_FIELDS, whereClause, orderBy, rowsToReturn))
            {
                while (reader.Read())
                {
                    list.Add(ConvertToEntityFromDataReader(reader));
